...
export default function CryptoSavingsCarousel() {
  return (
    <section className="relative py-24 overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 dark:from-gray-950 dark:via-purple-950/30 dark:to-black">
      {/* Background Glow */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 text-center">
        <div className="mb-16">
          ...
          <h2 className="text-5xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 mb-4">
        ...
        <div className="relative">
          <div className="flex gap-8 animate-infinite-scroll">
            {carouselItems.map((item, index) => (
              <div key={`${item.coin}-${index}`} className="shrink-0 w-96 group">
                <div className="relative h-full">
                  <div className={`absolute inset-0 bg-gradient-to-r ${item.gradient} opacity-60 blur-3xl group-hover:opacity-80 group-hover:blur-2xl transition-all duration-700`} />
                  ...
                  <div className="relative bg-white/95 dark:bg-gray-900/95 backdrop-blur-2xl border border-white/30 dark:border-gray-700 rounded-3xl p-10 shadow-2xl hover:shadow-3xl transition-all duration-500 hover:-translate-y-6 hover:scale-105">
                    {item.isNew && (
                      <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-amber-400 to-orange-600 text-black font-extrabold px-8 py-3 rounded-full text-sm shadow-2xl animate-bounce">
                        NEW USER BONUS
                      </div>
                    )}
                    ...
                    <div className={`text-5xl md:text-6xl font-black mb-3 bg-clip-text text-transparent bg-gradient-to-r ${item.gradient}`}>
                      {item.rate}
                    </div>
                    ...
                    <div className={`mt-8 h-1.5 rounded-full bg-gradient-to-r ${item.gradient} opacity-70 group-hover:opacity-100 transition-opacity`} />
                  </div>
                </div>
              </div>
            ))}
